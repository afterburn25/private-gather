...
$errors = [];
$need = [
    'index.php','private-gather.php','.htaccess','install/index.php','install/self-hosted-index.php','install/lib.php','install/runtime.php',
    'install/schema.sql','install/community-schema.php','install/product-completion-schema.php','docs/INSTALLATION.md','storage/app','bootstrap/cache',
];
foreach ($need as $path) if (! file_exists($base.'/'.$path)) $errors[] = 'Missing '.$path;
...
$hostedInstaller = (string) @file_get_contents($base.'/install/index.php');
$selfHostedInstaller = (string) @file_get_contents($base.'/install/self-hosted-index.php');
$installLib = (string) @file_get_contents($base.'/install/lib.php');
$runtime = (string) @file_get_contents($base.'/install/runtime.php');
...
$productSchema = (string) @file_get_contents($base.'/install/product-completion-schema.php');
...
$assertions = [
    ...
    ['browser installer registers invite token data hardening', str_contains($hostedInstaller, '2026_08_18_021500_hash_existing_event_invitation_tokens') && str_contains($selfHostedInstaller, '2026_08_18_021500_hash_existing_event_invitation_tokens')],
    ...
    ['supplemental installer creates tenant membership levels', str_contains($communitySchema, 'CREATE TABLE IF NOT EXISTS tenant_membership_levels') && str_contains($communitySchema, 'profile_eligibility') && str_contains($communitySchema, 'event_discount_percent')],
    ['supplemental installer creates membership renewal terms', str_contains($communitySchema, 'CREATE TABLE IF NOT EXISTS tenant_membership_terms') && str_contains($communitySchema, 'cancel_at_period_end') && str_contains($communitySchema, 'current_period_ends_at')],
    ['supplemental installer registers membership level migration', str_contains($communitySchema, '2026_08_22_233000_create_tenant_membership_levels')],
    ['product completion installer creates social/privacy data', str_contains($productSchema, 'CREATE TABLE IF NOT EXISTS user_connections') && str_contains($productSchema, 'CREATE TABLE IF NOT EXISTS user_privacy_settings') && str_contains($productSchema, 'CREATE TABLE IF NOT EXISTS reviews')],
    ['product completion installer creates CRM and consent-aware growth data', str_contains($productSchema, 'CREATE TABLE IF NOT EXISTS tenant_member_tags') && str_contains($productSchema, 'CREATE TABLE IF NOT EXISTS marketing_contacts') && str_contains($productSchema, 'email_opt_in_at') && str_contains($productSchema, 'sms_opt_in_at')],
    ['product completion installer creates marketplace accounting data', str_contains($productSchema, 'CREATE TABLE IF NOT EXISTS marketplace_ledger_entries') && str_contains($productSchema, 'platform_fee_cents') && str_contains($productSchema, 'CREATE TABLE IF NOT EXISTS payouts') && str_contains($productSchema, 'CREATE TABLE IF NOT EXISTS refunds')],
    ['product completion installer creates domain/trust data', str_contains($productSchema, 'CREATE TABLE IF NOT EXISTS domain_orders') && str_contains($productSchema, 'CREATE TABLE IF NOT EXISTS trust_cases')],
    ['product completion installer registers completion migration', str_contains($productSchema, '2026_08_23_010000_create_product_completion_suite')],
...
foreach ($assertions as [$label, $ok]) if (! $ok) $errors[] = 'Failed assertion: '.$label;
foreach (glob($base.'/database/migrations/*.php') ?: [] as $file) {
    $name = basename($file, '.php');
    if (! str_contains($schema, $name) && ! str_contains($hostedInstaller, $name) && ! str_contains($selfHostedInstaller, $name) && ! str_contains($communitySchema, $name) && ! str_contains($productSchema, $name)) {
