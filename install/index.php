...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (! installer_requirements_pass($checks)) throw new RuntimeException('Server requirements are not satisfied. Resolve the failed checks before installing.');

        $edition = strtolower(trim((string) ($_POST['edition'] ?? 'hosted')));
        if (! in_array($edition, ['hosted', 'self_hosted'], true)) throw new InvalidArgumentException('Choose a valid Private Gather edition.');
        ...
        if ($rootDomain === '' || ! preg_match('/^[a-z0-9.-]+$/', $rootDomain)) throw new InvalidArgumentException('Platform/root domain is invalid.');
        if ($edition === 'self_hosted' && trim((string) ($_POST['organization_name'] ?? '')) === '') throw new InvalidArgumentException('Organization name is required for Self-Hosted Edition.');
        if (! in_array((string) ($_POST['organization_type'] ?? ''), ['club', 'organizer', 'private_host'], true)) throw new InvalidArgumentException('Choose a valid organization type.');
        if (! in_array((string) ($_POST['self_hosted_visibility'] ?? ''), ['private', 'public'], true)) throw new InvalidArgumentException('Choose a valid Self-Hosted visibility mode.');
        if (! in_array((string) ($_POST['self_hosted_registration'] ?? ''), ['open', 'approval', 'disabled'], true)) throw new InvalidArgumentException('Choose a valid Self-Hosted registration mode.');
        ...
        if ($tenantId !== null) $installInput['self_hosted_tenant_id'] = $tenantId;
...
<div class="card"><h2>1. Choose Edition</h2><div class="grid"><div class="full"><label>Private Gather edition</label><select name="edition" id="edition"><option value="hosted" <?php echo $data['edition']==='hosted'?'selected':''; ?>>Hosted Edition — multi-organization SaaS</option><option value="self_hosted" <?php echo $data['edition']==='self_hosted'?'selected':''; ?>>Self-Hosted Edition — one private organization</option></select></div></div><div class="edition-grid" style="margin-top:14px"><div class="edition-card"><strong>Hosted Edition</strong><span class="help">Multiple clubs/organizers, Create Website, tenant isolation, hosted subdomains, marketplace and platform administration.</span></div><div class="edition-card"><strong>Self-Hosted Edition</strong><span class="help">One organization on the customer's own hosting. No wildcard subdomains or multi-site SaaS layer. Private/member-only access can be enforced.</span></div></div></div>
<div class="card"><h2>2. Server Requirements</h2><?php foreach ($checks as [$label,$ok]): ?><div class="req"><span><?php echo h($label); ?></span><strong class="<?php echo $ok?'pass':'fail'; ?>"><?php echo $ok?'PASS':'FAIL'; ?></strong></div><?php endforeach; ?><p class="help">The deployable package must include Composer's <code>vendor/</code> directory before the application can run.</p></div>
<div class="card"><h2>3. Website & Domain</h2><div class="grid"><div class="full"><label>Application / brand name</label><input name="app_name" required value="<?php echo h((string)$data['app_name']); ?>"></div><div class="full"><label>Application URL</label><input name="app_url" required value="<?php echo h((string)$data['app_url']); ?>"><div class="help">For a subdirectory install include the full path, for example <strong>https://example.com/private-gather</strong>.</div></div><div><label>Root domain</label><input name="platform_root_domain" required value="<?php echo h((string)$data['platform_root_domain']); ?>"><div class="help">Hosted: root for customer subdomains. Self-Hosted: this installation's domain.</div></div><div><label>Timezone</label><select name="timezone"><option value="America/Chicago" selected>America/Chicago</option><option value="UTC">UTC</option><option value="America/New_York">America/New_York</option><option value="America/Denver">America/Denver</option><option value="America/Los_Angeles">America/Los_Angeles</option></select></div></div></div>
<div class="card"><h2>4. Self-Hosted Organization</h2><p class="help">These settings are used only when Self-Hosted Edition is selected. The installer creates this organization automatically and makes the first administrator its owner.</p><div class="grid"><div class="full"><label>Organization / club name</label><input name="organization_name" value="<?php echo h((string)$data['organization_name']); ?>" placeholder="My Private Club"></div><div><label>Organization type</label><select name="organization_type"><option value="private_host" <?php echo $data['organization_type']==='private_host'?'selected':''; ?>>Private host</option><option value="club" <?php echo $data['organization_type']==='club'?'selected':''; ?>>Club / venue</option><option value="organizer" <?php echo $data['organization_type']==='organizer'?'selected':''; ?>>Event organizer</option></select></div><div><label>Website visibility</label><select name="self_hosted_visibility"><option value="private" <?php echo $data['self_hosted_visibility']==='private'?'selected':''; ?>>Private — login required</option><option value="public" <?php echo $data['self_hosted_visibility']==='public'?'selected':''; ?>>Public website</option></select></div><div><label>Member registration</label><select name="self_hosted_registration"><option value="approval" <?php echo $data['self_hosted_registration']==='approval'?'selected':''; ?>>Approval required</option><option value="open" <?php echo $data['self_hosted_registration']==='open'?'selected':''; ?>>Open registration</option><option value="disabled" <?php echo $data['self_hosted_registration']==='disabled'?'selected':''; ?>>Disabled</option></select></div></div></div>
<div class="card"><h2>5. Database</h2><div class="grid"><div><label>Database host</label><input name="db_host" required value="<?php echo h((string)$data['db_host']); ?>"></div><div><label>Database port</label><input name="db_port" type="number" required value="<?php echo h((string)$data['db_port']); ?>"></div><div><label>Database name</label><input name="db_database" required value="<?php echo h((string)$data['db_database']); ?>"></div><div><label>Database username</label><input name="db_username" required value="<?php echo h((string)$data['db_username']); ?>"></div><div class="full"><label>Database password</label><input name="db_password" type="password" value=""></div><div class="full check"><input id="db_create" name="db_create" type="checkbox" value="1" <?php echo !empty($data['db_create'])?'checked':''; ?>><div><label for="db_create" style="margin:0">Create database automatically when permitted</label><div class="help">If your hosting account already created the database, Private Gather will use the named database.</div></div></div></div></div>
<div class="card"><h2>6. Administrator</h2><div class="grid"><div><label>Name</label><input name="admin_name" required value="<?php echo h((string)$data['admin_name']); ?>"></div><div><label>Email</label><input name="admin_email" type="email" required value="<?php echo h((string)$data['admin_email']); ?>"></div><div><label>Password</label><input name="admin_password" type="password" required minlength="12"></div><div><label>Confirm password</label><input name="admin_password_confirm" type="password" required minlength="12"></div></div></div>
<div class="card"><h2>7. Install</h2><p class="note">The same Private Gather core is installed for both editions. Edition-specific capabilities are activated by configuration, so future shared features and security fixes can ship to Hosted and Self-Hosted together.</p><button class="button" type="submit" <?php echo installer_requirements_pass($checks)?'':'disabled'; ?>>Install Private Gather</button></div></form><?php endif; ?>
<div class="footer">Private Gather Unified Core · Hosted Edition + Self-Hosted Edition</div></main></body></html>