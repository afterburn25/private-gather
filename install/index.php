...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Clear stale framework cache files without requiring Artisan.
...
<div class="card">
  ...
  <p><a class="button" style="display:inline-block;text-decoration:none" href="<?php echo h(installer_app_href()); ?>">Open Website</a></p>
</div>
<?php else: ?>
...
<p class="help">The deployable package must include Composer's <code>vendor/</code> directory before the application can run. The browser installer itself is standalone, but installation is blocked if the Laravel runtime is missing.</p>
</div>

<div class="card"><h2>2. Website & Domain</h2><div class="grid">
<div class="full"><label>Brand / platform name</label><input name="app_name" required value="<?php echo h((string)$data['app_name']); ?>"></div>
<div class="full"><label>Application URL</label><input name="app_url" required value="<?php echo h((string)$data['app_url']); ?>"><div class="help">For a root install this auto-detects the domain. For a subdirectory install it preserves the full base path, for example <strong>https://demo.privoralabs.com/private-gather</strong>.</div></div>
<div><label>Private Gather hosted-domain root</label><input name="platform_root_domain" required value="<?php echo h((string)$data['platform_root_domain']); ?>"><div class="help">Client Private Gather subdomains become <code>client.&lt;this-domain&gt;</code>.</div></div>
...
<div class="full check"><input id="db_create" name="db_create" type="checkbox" value="1" <?php echo !empty($data['db_create'])?'checked':''; ?>><div><label for="db_create" style="margin:0">Create database automatically when permitted</label><div class="help">If your host already created the database (common on cPanel), leave the box checked or unchecked; the installer will use the named database. CREATE DATABASE requires permission from your database account.</div></div></div>
</div></div>

<div class="card"><h2>4. Private Gather Administrator</h2><div class="grid">
...
<div class="card"><h2>5. Install</h2><p class="note">The installer will inject the complete database schema, create the first administrator, generate a unique application key, write <code>.env</code>, clear stale cache files, create an installation receipt/lock, then remove <code>/install</code>.</p><button class="button" type="submit" <?php echo installer_requirements_pass($checks)?'':'disabled'; ?>>Install Private Gather</button></div>
</form>
<?php endif; ?>
<div class="footer">Private Gather can be installed in a domain/subdomain document root or inside a web subdirectory such as <code>/private-gather</code>.</div>
</main></body></html>
