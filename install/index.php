...
?><!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Private Gather Hosted Installer</title>
...
<body><main class="wrap"><div class="brand installer-brand"><img src="<?php echo h(installer_asset_href('assets/branding/private-gather-logo.png')); ?>" alt="Private Gather"><span>PRIVATE GATHER · HOSTED INSTALLER</span></div><section class="hero"><h1>Install Private Gather Hosted</h1><p>This package is the multi-organization Private Gather platform for clubs, organizers, tenant websites, marketplace discovery and platform administration. Self-Hosted is distributed as a separate base with its own installer.</p></section>
<?php if ($success): ?><div class="card"><div class="alert good"><strong>Installation complete.</strong> Private Gather Hosted is configured and ready.</div><p class="note"><?php echo $deleteOk ? 'The /install folder was removed automatically.' : 'The installer was locked, but this host prevented automatic folder removal/rename. Access is disabled; remove the install folder manually when convenient.'; ?></p><p><a class="button" style="display:inline-block;text-decoration:none" href="<?php echo h(installer_app_href()); ?>">Open Private Gather</a></p></div>
...
<div class="card"><h2>2. Platform & Domain</h2><div class="grid"><div class="full"><label>Platform / brand name</label><input name="app_name" required value="<?php echo h((string)$data['app_name']); ?>"></div><div class="full"><label>Application URL</label><input name="app_url" required value="<?php echo h((string)$data['app_url']); ?>"><div class="help">For a subdirectory install include the full path, for example <strong>https://example.com/private-gather</strong>.</div></div><div><label>Platform root domain</label><input name="platform_root_domain" required value="<?php echo h((string)$data['platform_root_domain']); ?>"><div class="help">Used for hosted tenant subdomains and central platform routing.</div></div><div><label>Timezone</label><select name="timezone"><option value="America/Chicago" selected>America/Chicago</option><option value="UTC">UTC</option><option value="America/New_York">America/New_York</option><option value="America/Denver">America/Denver</option><option value="America/Los_Angeles">America/Los_Angeles</option></select></div></div></div>
