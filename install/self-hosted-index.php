...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (! installer_requirements_pass($checks)) throw new RuntimeException('Server requirements are not satisfied. Private Gather attempted to create and repair its runtime directories automatically; any remaining failure requires the PHP/web-server account to own or be allowed to modify the affected path.');
        ...
        if ($rootDomain === '' || ! preg_match('/^[a-z0-9.-]+$/', $rootDomain)) throw new InvalidArgumentException('Site domain is invalid.');
        if (trim((string) ($_POST['organization_name'] ?? '')) === '') throw new InvalidArgumentException('Organization name is required.');
        ...
        if (! in_array((string) ($_POST['self_hosted_visibility'] ?? ''), ['private', 'public'], true)) throw new InvalidArgumentException('Choose a valid visibility mode.');
        if (! in_array((string) ($_POST['self_hosted_registration'] ?? ''), ['open', 'approval', 'disabled'], true)) throw new InvalidArgumentException('Choose a valid registration mode.');
        ...
        $installInput = [
            ...
            'organization_type' => (string) $_POST['organization_type'],
            'self_hosted_visibility' => (string) $_POST['self_hosted_visibility'],
            'self_hosted_registration' => (string) $_POST['self_hosted_registration'],
        ...
        if ($tenantId === null) throw new RuntimeException('Self-Hosted organization provisioning did not return a tenant ID.');
...
?><!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Private Gather Self-Hosted Installer</title>
...
<body><main class="wrap"><div class="brand installer-brand"><img src="<?php echo h(installer_asset_href('assets/branding/private-gather-logo.png')); ?>" alt="Private Gather"><span>PRIVATE GATHER · SELF-HOSTED INSTALLER</span></div><section class="hero"><h1>Install Private Gather Self-Hosted</h1><p>This package is the dedicated single-organization Private Gather base for a club, organizer or private host running on customer-owned hosting. It does not install the multi-tenant Hosted platform.</p></section>
<?php if ($success): ?><div class="card"><div class="alert good"><strong>Installation complete.</strong> Private Gather Self-Hosted is configured and your organization has been created.</div><p class="note"><?php echo $deleteOk ? 'The /install folder was removed automatically.' : 'The installer was locked, but this host prevented automatic folder removal/rename. Access is disabled; remove the install folder manually when convenient.'; ?></p><p><a class="button" style="display:inline-block;text-decoration:none" href="<?php echo h(installer_app_href()); ?>">Open Private Gather</a></p></div>
