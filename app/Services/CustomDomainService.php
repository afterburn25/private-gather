...
final class CustomDomainService{
 ...
 public function dnsInstructions(TenantDomain $domain):array{return ['domain'=>$domain->domain,'cname_target'=>config('platform.domain_target'),'verification_record'=>config('platform.domain_verification.txt_prefix').'.'.$domain->domain,'verification_value'=>$domain->verification_token,'note'=>'For an apex/root domain, use the ALIAS/ANAME/CNAME-flattening option offered by the DNS provider, or configure the A/AAAA target supplied by the hosting edge.'];}
}