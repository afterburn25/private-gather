...
@foreach($page->sections as $section)
 ...
 @switch($section->type)
  @case('hero')
   <section class="pg-page-hero tenant-site-hero tenant-section tenant-section-hero">
    <div class="tenant-shell pg-grid pg-grid-2" style="align-items:center">
     <div class="tenant-hero-copy">
      ...
      <div class="pg-trust-row">
       ...
       <span class="pg-pill">{{$tenant->isClub() ? 'Private club community' : 'Private community'}}</span>
      </div>
      @if(data_get($content,'primary_label'))
       <div class="pg-actions" style="margin-top:24px"><a class="tenant-button tenant-button-primary" href="{{\App\Support\MountUrl::to(data_get($content,'primary_url','#'))}}">{{$renderer->render((string)data_get($content,'primary_label'),$templateContext)}}</a></div>
      @endif
     </div>
     <div class="pg-media tenant-hero-media" style="min-height:390px">
  ...
  @case('event_grid')
   <section class="pg-section tenant-section tenant-section-events"><div class="tenant-shell"><div class="pg-section-head"><div><span class="pg-eyebrow">UPCOMING</span><h2>{{$heading}}</h2></div><a class="text-link" href="{{route('site.events')}}">View all →</a></div><div class="pg-grid pg-grid-3">
  ...
  @case('cta')
   <section class="pg-section tenant-section tenant-section-cta"><div class="tenant-shell"><div class="pg-card tenant-cta" style="padding:clamp(28px,5vw,50px)"><div class="pg-grid pg-grid-2" style="align-items:center"><div><span class="pg-eyebrow">MEMBERSHIP & COMMUNITY</span><h2 style="font-size:clamp(2rem,4vw,3.5rem);letter-spacing:-.05em;margin:.45rem 0">{{$heading}}</h2><p class="pg-lead">{{$body}}</p></div><div class="pg-actions" style="justify-content:flex-end"><a class="tenant-button tenant-button-primary" href="{{\App\Support\MountUrl::to(data_get($content,'button_url','#'))}}">{{$renderer->render((string)data_get($content,'button_label','Learn more'),$templateContext)}}</a></div></div></div></div></section>
   @break

  @case('image_text')
   <section class="pg-section tenant-section tenant-section-story"><div class="tenant-shell pg-grid pg-grid-2" style="align-items:center"><div class="pg-media tenant-story-media">@if(data_get($content,'image_url'))<img src="{{\App\Support\MountUrl::to(data_get($content,'image_url'))}}" alt="{{data_get($content,'image_alt','')}}">@endif</div><div class="pg-card tenant-story-copy"><span class="pg-eyebrow">ABOUT THIS COMMUNITY</span><h2>{{$heading}}</h2><div class="prose">{!!nl2br(e($body))!!}</div></div></div></section>
   @break

  @default
   <section class="pg-section tenant-section tenant-section-content"><div class="tenant-shell narrow"><div class="pg-card"><h2>{{$heading}}</h2>@if(data_get($content,'image_url'))<div class="pg-media" style="margin:18px 0"><img src="{{\App\Support\MountUrl::to(data_get($content,'image_url'))}}" alt="{{data_get($content,'image_alt','')}}"></div>@endif @if($body)<div class="prose">{!!nl2br(e($body))!!}</div>@endif</div></div></section>
 @endswitch
@endforeach
