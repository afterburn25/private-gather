...
<div class="panel-head"><div><div class="eyebrow">ORGANIZER CONTROL CENTER</div><h1>{{$tenant->name}}</h1><p class="muted">Manage your hosted website, events, branding, domains and staff from one place.</p>@if($publicUrl)<p class="muted">Public address: <a class="text-link" target="_blank" rel="noopener" href="{{$publicUrl}}">{{$publicUrl}}</a></p>@endif</div><div class="row-actions">@if($publicUrl)<a class="btn primary" target="_blank" rel="noopener" href="{{$publicUrl}}">Open Public Website</a>@endif<a class="btn" target="_blank" rel="noopener" href="{{route('organizations.index',['preview'=>$tenant->id])}}">Preview Website</a><a class="btn" href="{{route('organizations.index')}}">My Sites</a></div></div>
